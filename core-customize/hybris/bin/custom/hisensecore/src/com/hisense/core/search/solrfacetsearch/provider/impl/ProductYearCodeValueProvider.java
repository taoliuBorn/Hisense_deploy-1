...
public class ProductYearCodeValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider {
    ...
    public Collection<FieldValue> getFieldValues(IndexConfig indexConfig, IndexedProperty indexedProperty, Object o) throws FieldValueProviderException {
        ...
        if (StringUtils.isNotBlank(year)) {
            ...
            fieldValues.addAll(createFieldValue(year, indexedProperty));
            return fieldValues;
        } else {
            return Collections.emptyList();
        }
    ...
    private Collection<? extends FieldValue> createFieldValue(String year, IndexedProperty indexedProperty) {
        final List<FieldValue> fieldValues = new ArrayList<FieldValue>();
        Collection<String> fieldNames = fieldNameProvider.getFieldNames(indexedProperty, null);
        for (final String fieldName : fieldNames)
        {
            fieldValues.add(new FieldValue(fieldName, year));
        }
        return fieldValues;
    }
